...
module registers #(
    ...
    // R29, R30, R31 are read-only Hardware IDs
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            if (lsu_we && (lsu_rd < 29)) registers[lsu_warp_id][lsu_rd] <= lsu_data;
            if (is_writing) registers[warp_id][decoded_rd_address] <= write_data;
        end
    end
endmodule